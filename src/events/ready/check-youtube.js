    ...
    async function checkYoutube() {
        try {
            ...
            for (const notificationConfig of notificationConfigs) {
                ...
                if (!lastCheckedVid || (latestVideo.id.split(':')[2] !== lastCheckedVid.id && new Date(latestVideo.pubDate) > new Date(lastCheckedVid.pubDate))){
                    const targetGuild = client.guilds.cache.get(notificationConfig.guildId) || (await client.guilds.fetch(notificationConfig.guildId))
                    ...
                    notificationConfig.save()
                        .then(() => {
                            const targetMessage = notificationConfig.customMessage
                                ...
                                ?.replace('{CHANNEL_NAME', feed.title) || `New video by ${feed.title}\n${latestVideo.link}`;
